...
public abstract class AbstractLevel extends ScreenAdapter {
	...
	public void render(float delta) {
		...
		bitMapFontName.draw(game.getBatch(), "Score: " + game.getState().getScore(), 0 + camera.position.x - 350,
				0 + camera.position.y + 280);
		bitMapFontName.draw(game.getBatch(), "Level: " + game.getState().getLvl(), 0 + camera.position.x - 500,
				0 + camera.position.y + 280);
		game.getBatch().end();

		/*
		 * Get the time it took to render the last frame and make sure our delta time is
		 * never larger than 0.0166666666666667 (60 FPS). This will ensure that on fast
		 * OS the game at only 60 FPS.
		 */
